...
int main(int argc, char * argv[])
{
	...
	//Because of LMem
	int dataSizeBytes = calc_align(size, 384)*sizeof(float);
    ...
    writeact.param_nbytes = dataSizeBytes;
    writeact.instream_cpu_to_lmem = mat_b;
    MatMatMultiply_writeLMem_run(engine, &writeact);

    MatMatMultiply_actions_t actions;
	actions.param_matrixLength = calc_align(size, 384);
	actions.param_n = n;
